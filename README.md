...
Requires PHP 8.5 and the runtime dependencies in `composer.json`. The canonical namespace is `Kumwe\Administrator\Contract\`. Published baseline: 0.1.0. This branch records the 0.2.0 successor for publication after merge and the complete package gate. Independently verify that published artifact before App adoption.
