...
/** Validate shipped governance contracts without a consumer checkout or runtime dependency. */
$root = dirname(__DIR__);
$read = static fn (string $path): array => json_decode(
    file_get_contents($root . '/' . $path), true, 512, JSON_THROW_ON_ERROR,
);
...
if ($matches[1] !== $sections || !str_starts_with($record, "---\n")) {
    throw new RuntimeException('Release record must retain front matter and the eight ordered contract sections.');
}
